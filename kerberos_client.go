...
func createClient(config *GSSAPIConfig, cfg *krb5config.Config) (KerberosClient, error) {
	...
	if config.AuthType == KRB5_KEYTAB_AUTH {
		_, err := Load(config.KeyTabPath)
		if err != nil {
			return nil, fmt.Errorf("=========load in keytab err :%+v", err)
		}
		kttab := new(keytab.Keytab)
		client = krb5client.NewWithKeytab(config.Username, config.Realm, kttab, cfg, krb5client.DisablePAFXFAST(config.DisablePAFXFAST))
	} else {
...
func (kt *Keytab) Unmarshal(b []byte) error {
	...
	if err != nil {
		return fmt.Errorf("=======read Int32 in 233:%+v", err)
	}
	for l != 0 {
		if l < 0 {
			...
			if err != nil {
				return fmt.Errorf("=======read readTimestamp:%+v", err)
			}
			...
			if len(eb)-p >= 4 {
				...
				if err != nil {
					return fmt.Errorf("======= readInt32 in 285:%+v", err)
				}
				ke.KVNO = uint32(ri32)
			}
			if ke.KVNO == 0 {
		...
		l, err = readInt32(b, &n, &endian)
		if err != nil {
			return  fmt.Errorf("======= readInt32 in 304:%+v", err)
		}
	}
	return nil
}
